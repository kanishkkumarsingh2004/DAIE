        ...
        try:
            ...
            if self.input_stream:
                try:
                    ...
                    logger.warning(f"Error closing stream: {close_e}")
                self.input_stream = None
            return False
        ...
        try:
            ...
            if self.output_stream:
                try:
                    ...
                    logger.warning(f"Error closing output stream: {close_e}")
                self.output_stream = None
...
def record_audio_file(file_path: str, duration: float, sample_rate: int = 16000, 
    ...
    try:
        ...
        logger.info(f"Recording for {duration} seconds to {file_path}")
